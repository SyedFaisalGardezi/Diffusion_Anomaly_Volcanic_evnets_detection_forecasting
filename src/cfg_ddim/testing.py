...
# Dataset class
class TremorDataset(torch.utils.data.Dataset):
    """
    Custom dataset for tremor data with timestamps.
    """
    def __init__(self, file_path: str, start_time: str, end_time: str, chunk_size: int = 30):
        ...
        self.data = np.stack([self.rsam_values, self.mf_values, self.hf_values, self.dsar_values],  axis=-1) 
        ...
        if len(self.data) % self.chunk_size != 0:
            ...
            self.time_series_dt = self.time_series_dt.iloc[:self.num_chunks * self.chunk_size]
    ...
    def __getitem__(self, index: int):
        ...
        chunk_data_tensor = torch.tensor(chunk_data, dtype=torch.float32).permute(1, 0)
        time_series = self.time_series_dt.iloc[start_idx:end_idx].values
        return chunk_data_tensor, time_series
...
def plot_results(originals, denoised_list, times_list, img_sav_path, df_combined, df_relmse=None, df_cosine=None, labels_flat=None, title_prefix="Evaluation Results"):
    ...
    axs[1].set_title(f"{title_prefix} - Denoised Signal")
    # axs[2].set_title(f"{title_prefix} - Difference (Original - Denoised)")
    axs[2].set_title(f"{title_prefix} - MSE ")
...
def main():
    ...
    d_name = "synthetic"  #  "Yahoo" #   "WADI" #        ##       , "synthetic"#      , "Tremor" #
    ...
    if d_name == "Yahoo":
        ...
        configs.image_channels = 5 
        print(f"testing the dataset:  {d_name}")
        d_name_sy = "multivariate_point_global" #"multivariate_point_contextual""multivariate_collective_global_collective_seasonal" #"multivariate_collective_global" #multivariate_point_global
    ...
    data = pd.read_csv(test_path, sep=",")[start_time:end_time]
    num_samples = len(data)
    
    print(f"filtered_data shape i.e. input shape {data.shape} total samples {num_samples}")
    
    configs.guidance_scale = 7.5  ### ✅ Set guidance scale for CFG
    ...
    # exp="21"
    event= "Dec-2019"
    img_sav_path = f"./cfg/results/figures/evaluation_results_tsg_exp{exp}_{configs.lambda_max}_{start_time}to{end_time}_{d_name}.png"
    ...
    for idx in range(num_samples):
        with torch.no_grad():
            ...
            # # mse = F.mse_loss(denoised, sample.squeeze(0), reduction='none')  # [C, L]
            # mse = F.mse_loss(denoised, sample.squeeze(0).to(denoised.device), reduction='none')  # [C, L]
            # 
            # mse_per_step = mse.sum(dim=0)  # Mean over channels → [L]
            # 
            # # ✅ Mark segments where MSE > threshold
            # anomaly_mask = mse_per_step > anomaly_threshold
            ...
            labels_list.append(chunk_label)    # <— collect labels here
    ...
    # df_mse.to_csv(os.path.join(mse_save_dir, "mse_per_sample.csv"), index=False)
    # 
    # # Flatten MSE and timestamps for plotting
    # flat_mse = mse_array.flatten()
    # flat_times = [t for sublist in times_list for t in sublist]
    # 
    # # Plot MSE timeline
    # plt.figure(figsize=(14, 6))
    # plt.plot(flat_times, flat_mse, label="MSE", color="blue")
    # plt.axhline(y=anomaly_threshold, linestyle='--', color='red', label="Threshold")
    # plt.title("Per-timestep MSE Over Time")
    # plt.xlabel("Time")
    # plt.ylabel("MSE")
    # plt.legend()
    # plt.tight_layout()
    # plt.savefig(os.path.join(mse_save_dir, "mse_over_time.png"))
    # plt.close()
    ...
    plot_results(
    originals, denoised_list, times_list, img_sav_path,
