...
def plot_results(originals, denoised_list, times_list, img_sav_path, df_combined, df_relmse=None, df_cosine=None, labels_flat=None, title_prefix="Evaluation Results"):
    ...
    # axs[3].plot(times_cat_trimmed, df_combined_trim, '-', color='red', label="Combined Score")
    if df_relmse_trim is not None:
        axs[2].plot(times_cat_trimmed, df_relmse_trim, '--', color='green', label="RelMSE")
    if df_cosine_trim is not None:
        axs[2].plot(times_cat_trimmed, df_cosine_trim, '--', color='orange', label="Cosine Dist")
    axs[2].axhline(y=0.3, color='gray', linestyle='--', label="Threshold")
...
def main():
    ...
    # test_path = "./data_external/time_series_datasets/WaDi/WADI.A1_9 Oct 2017/df_attack_processed.csv"
     #
    # 
    
    # "./cfg/data/processed/synth_data/pattern_seasonal/train.csv"
    ...
    d_name = 'SWAT'  #    "synthetic"  # "WADI"  #  "Tremor" #         "Yahoo"   #   
    
    if d_name == "Tremor":
        
        # start_time = "2012-05-01" ## eruption date 2012 08 04 16 52 00 and 2013 08 19
        # end_time = "2012-08-10"
        
        # start_time = "2013-06-1"  ## eruption date  2013 08 19 and 2013 10 03
        # end_time = "2013-10-10"
#         
#         start_time = "2016-01-1"
#         end_time = "2016-05-05"

        # start_time = "2019-11-1"
        # end_time = "2019-12-20"
        
        start_time = "2007-02-01"  #"2007-03-18"
        end_time = "2007-03-28"
    ...
    configs.chunk_size = 100 #num_samples #
    ensamble = 2
    WINDOW_SIZE = 6 ## for anomaly score smoothing
    ...
    if d_name == "Yahoo":
        ...
        print(f"testing the dataset:  {d_name}")
        test_path = "./cfg/data/processed/WaDi/WADI.A1_9 Oct 2017/df_attack_processed_norm.csv"
        ...
        print(f"testing the dataset:  {d_name}")
        # test_path = "./cfg/data/processed/SWat/test_data_with_anomaly_labels.csv"
        test_path = "./cfg/data/processed/SWat/test_data_labels_robust.csv"
        ...
        configs.image_channels = 5
        d_name_sy = "multivariate_collective_seasonal"  #multivariate_collective_seasonal
        print(f"testing the dataset:  {d_name} for data name {d_name_sy}")
        #"multivariate_point_contextual""multivariate_collective_global_collective_seasonal" #"multivariate_collective_global" #multivariate_point_global  ## "multivariate_collective_trend"    #  multivariate_collective_seasonal  #  
        test_path = f"./cfg/data/processed/my_synth_gen/{d_name_sy}.csv"
...
#     if d_name == "Tremor":
#         df_test = pd.read_csv(test_path, sep=",", parse_dates=['time'])
#         df_test['time'] = pd.to_datetime(df_test['time'])
# 
#         # Convert your start/end strings to Timestamp
#         t0 = pd.to_datetime(start_time)
#         t1 = pd.to_datetime(end_time)
#     else:
#         
#     
# 
#     # Boolean mask on the time column
#     mask = (df_test['time'] >= t0) & (df_test['time'] <= t1)
#     data = df_test.loc[mask].reset_index(drop=True)
# 
#     num_samples = len(data)
#     print(f"filtered_data shape i.e. input shape {data.shape} total samples {num_samples}")
#     num_samples = len(data)
#     
#     print(f"filtered_data shape i.e. input shape {data.shape} total samples {num_samples}")
#     
    wtsg = configs.guidance_scale = 4.0  ### ✅ Set guidance scale for CFG
    configs.lambda_max = 500 ## defines from where we should start the backward sampling
    configs.S = 2.0
    configs.T_MIN = 10
    configs.ALPHA = 1.0
    delta = 0.1 ## for AUC computation
    noise_type =  "gaussian" ## "simplex"#
    ...
    mse_save_dir = f"./cfg/results/mse_analysis/v{exp}_{d_name}"
    ...
    path_denoised =f"./cfg/results/denoised_signal/denoised_signal_exp{exp}_{configs.lambda_max}_{start_time}to{end_time}_{d_name}_{wtsg}_{noise_type}"
    os.makedirs(path_denoised, exist_ok=True)
    # print(f"shape of denoised list{}")
    # 1) convert each sig → 1-D numpy array
    signals = []
    for sig in denoised_list:
        # if it's a torch.Tensor, move to CPU and convert
        if hasattr(sig, "cpu"):
            arr = sig.cpu().numpy()
        else:
            arr = np.array(sig)
        # if multi-channel (C, L), pick one channel or flatten, e.g. first channel:
        # arr = arr[0]              # ← only if arr.ndim == 2
        # otherwise just flatten
        arr = arr.flatten()
        signals.append(arr)

    # 2) check they all have same length
    lengths = [len(s) for s in signals]
    if len(set(lengths)) != 1:
        raise ValueError(f"Signal lengths differ: {set(lengths)}")

    # 3) build the DataFrame: one column per signal
    data = {f"signal_{i}": s for i, s in enumerate(signals)}
    df = pd.DataFrame(data)

    # (optional) if you want a timestep column instead of the default 0…L-1 index:
    df.index.name = "timestep"
    df.reset_index(inplace=True)

    # 4) write to CSV
    out_csv = os.path.join(path_denoised, "denoised_signals.csv")
    df.to_csv(out_csv, index=False)

    print(f"Saved {len(signals)} signals (length {lengths[0]}) → {out_csv}")
    
    # 1) If you want to keep per-chunk anomaly scores, collect them in-loop:
    #    right after you compute `score` in the loop, add:
    #       score_list.append(score.detach().cpu().numpy())
    # Make sure you've declared `score_list = []` before the loop.
    try:
        score_list
    except NameError:
        score_list = []  # fallback if you forgot to collect; will be empty

    # 2) Per-chunk stacks (N, C, L)
    original_chunks = torch.stack(originals, dim=0).cpu().numpy()      # (N, C, L)
    denoised_chunks = torch.stack(denoised_list, dim=0).cpu().numpy()  # (N, C, L)

    # times & labels per chunk: keep as object array to preserve ragged lists (if any)
    times_chunks   = np.array(times_list, dtype=object)   # (N,)
    labels_chunks  = np.array(labels_list, dtype=object)  # (N,)
    if len(score_list) == len(denoised_list):
        score_chunks = np.stack(score_list, axis=0)       # (N, L)
    else:
        score_chunks = None

    # 3) Stitched full-length (concatenate along time axis)
    #    originals/denoised are [C, L] each → cat along dim=1 → [C, total_L]
    original_full = torch.cat(originals, dim=1).cpu().numpy()         # (C, total_L)
    denoised_full = torch.cat(denoised_list, dim=1).cpu().numpy()     # (C, total_L)

    #    times/labels are per-timestep vectors → concat 1D
    try:
        times_full  = np.concatenate([np.asarray(t) for t in times_list])   # (total_L,)
    except Exception:
        # Fallback to simple integer index if times are inconsistent
        total_L = sum(x.shape[-1] for x in originals)
        times_full = np.arange(total_L)

    labels_full = np.concatenate([np.asarray(lbl) for lbl in labels_list])  # (total_L,)

    if score_chunks is not None:
        score_full = np.concatenate(score_chunks, axis=0)                   # (total_L,)
    else:
        # If you want a default MSE score across channels:
        # compute per-chunk then concat
        score_full = np.concatenate([
            ((o.cpu().numpy() - d.cpu().numpy())**2).mean(axis=0)           # (L,)
            for o, d in zip(originals, denoised_list)
        ])

    # 4) Save everything into a single .npz
    npz_file_path = os.path.join(path_denoised, "denoised_ensemble_signals.npz")
    save_kwargs = dict(
        # per-chunk
        original_chunks=original_chunks,
        denoised_chunks=denoised_chunks,
        times_chunks=times_chunks,
        labels_chunks=labels_chunks,
        score_chunks=score_chunks if score_chunks is not None else np.array([], dtype=float),

        # stitched full series
    ...
    np.savez_compressed(npz_file_path, **save_kwargs)

    print(f"[SAVE] Per-chunk: original {original_chunks.shape}, denoised {denoised_chunks.shape}")
    print(f"[SAVE] Full: original {original_full.shape}, denoised {denoised_full.shape}")
    print(f"[SAVE] Times_full {times_full.shape}, Labels_full {labels_full.shape}, Score_full {score_full.shape}")
    print(f"[SAVE] Wrote: {npz_file_path}")
    ...
    # ——— 1) VALIDATION: collect scores & labels, pick best δ via evaluate() ———
    print("Evaluating on validation set to pick best δ...")
    val_scores, val_labels = evaluate_on_split(
        configs.dataset_val,
        sampler,
        relmse_weight=1.0,
        cosine_weight=1.0,
    ...
    print(f"  Validation: collected {len(val_scores)} time‐steps.")
    
    # ——— Optional smoothing of the raw anomaly score on VAL set ——————
        
    # median‐filter, center‐aligned, min_periods=1 to avoid NaNs at edges
    val_scores = (
        pd.Series(val_scores)
          .rolling(window=WINDOW_SIZE, center=True, min_periods=1)
          .median()
          .to_numpy() )


    # evaluate(...) with no validation_thresh will:
    #  • search 50 δ’s in [0,max(score)]
    #  • find the δ maximizing F1_K–AUC
    #  • return that δ in metrics['threshold']
    metrics_val, _, _ = evaluate(
        scores=val_scores,
        labels=val_labels,
        validation_thresh=None
    )
    best_delta = metrics_val['threshold']
    print(f"→ Selected best δ = {best_delta:.4f} (val F1_K–AUC = {metrics_val['f1']:.4f})")

    # ——— 2) TEST: collect scores & labels ——————————————————————————————
    print("Evaluating on test set with δ fixed from validation…")
    test_scores, test_labels = evaluate_on_split(
        configs.dataset,
        sampler,
        relmse_weight=1.0,
        cosine_weight=1.0,
    ...
    print(f"  Test: collected {len(test_scores)} time‐steps.")
    
    # ——— Same smoothing on the TEST set ———————————————————————————
    test_scores = ( pd.Series(test_scores).rolling(window=WINDOW_SIZE, center=True, min_periods=1)
          .median().to_numpy() )

    # ——— 3a) Compute F1_K–AUC at δ = best_delta via pak_protocol —————————
    area_f1, max_f1, k_of_max, preds_at_delta, fprs_at_delta, tprs_at_delta = pak_protocol(
        test_scores,
        test_labels,
        threshold=best_delta,
        max_k=100
    )
    print(f"Test • F1_K–AUC @ δ={best_delta:.4f}: {area_f1:.4f}")
    print(f"       best single‐K F1 = {max_f1:.4f} at K = {k_of_max}")

    # ——— 3b) Compute ROC_K–AUC (threshold‐free) via evaluate() ————————————
    # Calling evaluate with no validation_thresh returns (metrics, fpr_flat, tpr_flat).
    metrics_test, fpr_flat, tpr_flat = evaluate(
        scores=test_scores,
        labels=test_labels,
        validation_thresh=None
    )
    roc_k_auc_test = metrics_test['ROC/AUC']
    print(f"Test • ROC_K–AUC: {roc_k_auc_test:.4f}")

    # ——— 3c) Standard point‐wise ROC‐AUC for reference ————————————————
    roc_auc_standard = roc_auc_score(test_labels, test_scores)
    print(f"Test • Standard ROC-AUC: {roc_auc_standard:.4f}")

    # ——— 4) Save summary CSV —————————————————————————————————————
    summary_df = pd.DataFrame({
        'metric': ['F1K_AUC', 'ROC_K_AUC', 'ROC_AUC'],
        'value':  [area_f1, roc_k_auc_test, roc_auc_standard]
    })
    summary_df.to_csv(
        os.path.join(mse_save_dir, "metrics_summary.csv"),
        index=False
    )

    # ——— 5) Save full ROC “cloud” CSV ——————————————————————————————
    roc_cloud_df = pd.DataFrame({'fpr': fpr_flat, 'tpr': tpr_flat})
    roc_cloud_df.to_csv(
        os.path.join(mse_save_dir, "roc_cloud.csv"),
        index=False
    )

    # ——— 6) (Optional) Save per‐time‐step combined anomaly scores ————————
    pd.DataFrame(test_scores).to_csv(
        os.path.join(mse_save_dir, "anomaly_score_combined_test.csv"),
        index=False
    )

    print(f"All test metrics & CSVs saved to {mse_save_dir}")
